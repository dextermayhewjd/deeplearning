...
# wk4
parser.add_argument("--data-aug-hflip", action="store_true", help="Apply horizontal flipping data augmentation during training")
...
class CNN(nn.Module):
    ...
    def forward(self, images: torch.Tensor) -> torch.Tensor:
        ...
        x = torch.flatten(x, start_dim=1, end_dim=-1)  # Flatten each image in the batch

        ## Ensure the shape is [batch_size, 4096]
        assert x.shape[1] == 4096, f"Expected second dimension to be 4096 but got {x.shape[1]}"
...
def get_summary_writer_log_dir(args: argparse.Namespace) -> str:
    ...
    tb_log_dir_prefix = (
        ...
        f'momentum=0.9_' +
        ('hflip_' if args.data_aug_hflip else '') +  # Include 'hflip_' if args.data_aug_hflip is True
        f'run_'
    )
    i = 0
    while i < 1000:
